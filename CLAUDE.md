...
5. Pitch: pitch_mask.py on-pitch test (done, sampled), pitch_calibrate.py anchor homography (done for clipB - 4 anchors, all cross-check within 0.5 to 5 m - see Phase 3 findings; clipA has no anchors yet)
...
3. DONE for clipB, NOT STARTED for clipA (needs the owner for clipA): clipB has 4 anchors (130s, 180.1s, 197s, 270s) that all cross-check within 0.5 to 5.05 m, including the largest gap tested (140 s). `tracklet_pitch_xy.csv.gz` in data/clipB is ready to use. clipA needs its own 3+ anchors from scratch with `pitch_anchor_ui.py --run data/clipA`.
