...
- CLIPB CALIBRATION (2026-09-22): built up to 4 anchors (30s, 180.1s, 197s, 270s). 180.1s, 197s and 270s all cross-check cleanly with each other (0.5 to 4.2 m error over gaps up to 90 s) - real validation that the camera-motion propagation works. The 30s anchor was wildly inconsistent with all three (67 to 152 m error) both before and after relabeling its points with near/far touchline wording, ruling out a sign-convention mixup as the cause.
- ROOT CAUSE, CONFIRMED BY THE OWNER: the 30s anchor's frame shows the OPPOSITE goal from the other three (the ball, and the camera pan that follows it, had play at the far end of the pitch at that timestamp). Landmarks there were labeled with their standard "distance from the goal in view" meters, but that goal is not this calibration's X=0 reference - converting them would need the exact pitch length, which anchor-based calibration is deliberately designed not to assume. This is not fixable by relabeling; it needs a different anchor frame that shows the reference goal (the one visible at 180.1s/197s/270s). NEEDS THE OWNER: drop the 30s anchor and place a new one somewhere in clipB's first ~130s where play is near the reference goal, using `pitch_anchor_ui.py`'s Extract feature to grab that frame. The orientation-mismatch warning (above) will confirm agreement before you rely on it.
...
5. Pitch: pitch_mask.py on-pitch test (done, sampled), pitch_calibrate.py anchor homography (tool done, self-tested; 3 of 4 anchors placed on clipB via pitch_anchor_ui.py cross-check well - see Phase 3 findings. The 30s anchor shows the wrong goal and needs to be replaced; clipA has no anchors yet)
...
3. IN PROGRESS (needs the owner): clipB has 4 anchors (30s, 180.1s, 197s, 270s). 180.1s, 197s and 270s cross-check well (0.5 to 4.2 m over gaps up to 90 s). The 30s anchor shows the opposite goal from the other three (confirmed by the owner) - not a labeling convention issue, so relabeling didn't fix it. It needs to be replaced with a frame from clipB's first ~130s that shows the reference goal; `pitch_anchor_ui.py` now warns on save if a new anchor's orientation disagrees with the rest. clipA has no anchors yet.
