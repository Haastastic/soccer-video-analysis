...
- FIRST RUN ON REAL FOOTAGE (clipB, 2026-09-22): one anchor at 180.1s, 9 points, fit RMS 0.6 m, but only 6.0% of clipB's 95,664 tracklet rows fell within 10 s of it, so error elsewhere was unmeasured (cross_check needs a 2nd anchor).
- THREE ANCHORS (clipB, 2026-09-22): added anchors at 30s (10 points) and 270s (9 points). All three fit their own frame well (RMS 0.6 to 0.7 m) and coverage within 10s of an anchor rose to 19.1%. But cross_check is not uniformly good: 180.1s and 270s carry to each other cleanly (1.6 to 2.7 m error over a 90 s gap), while the 30s anchor is wildly inconsistent with both others (68 to 152 m error, gaps 150 to 240 s) - errors many times the length of the pitch itself, not a gradual drift.
- Diagnosed the 30s anchor as a bad point (not a camera-tracking problem): camera.csv has 400-inlier tracking on every frame in the run except frame 0 (the one weak frame already noted in Phase 1b), ruling out a tracking break between 30s and 180s/270s. The 3 anchors' homographies were compared by the sign of the linear part's determinant (orientation: a real camera mapping should keep the same handedness across frames from the same sideline) - 180.1s and 270s are both positive, 30s is negative. That flip, landing on exactly the anchor with the huge cross-check error, points to a labeling mistake on the 30s anchor (most likely a +Y/-Y mixup on one or more landmarks there), not the propagation method. NEEDS THE OWNER: re-examine or redo the 30s anchor's point labels in pitch_anchor_ui.py before trusting any pitch position derived from it. Until fixed, treat only the 180.1s and 270s anchors as reliable, which still leaves clipB's first ~130s without a nearby good anchor.
- `pitch_anchor_ui.py --run <run>` is a local-only browser UI (127.0.0.1, no external calls, since frames show minors) for placing anchors: click landmarks on a frame with a zoomable crosshair, quick-pick chips for the 9 standard Law-of-the-Game points fill in the pitch meters, live reprojection RMS, saves the same JSON `pitch_calibrate.py` reads.
...
5. Pitch: pitch_mask.py on-pitch test (done, sampled), pitch_calibrate.py anchor homography (tool done, self-tested; 3 anchors placed on clipB via pitch_anchor_ui.py, but the 30s one is likely mislabeled - see Phase 3 findings. 180.1s and 270s cross-check well; the rest of the clip, and clipA entirely, still need anchors)
...
3. IN PROGRESS (needs the owner): clipB has 3 anchors (30s, 180.1s, 270s) placed with pitch_anchor_ui.py. 180.1s and 270s cross-check well (1.6 to 2.7 m over 90 s). The 30s anchor is likely mislabeled (its homography has flipped orientation versus the other two, and 68 to 152 m cross-check error) - re-examine or redo its points before using it. After that: clipB's first ~130s still has no nearby good anchor, and clipA has none yet.
